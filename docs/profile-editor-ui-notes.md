...
button above the weighting tabs navigates directly to Gear Grades.
...
## Gear Grades testing view

The top-level Gear Grades page uses the compiled affix and item catalogs with
...
top-five Components table and a top-five Augments table. Components distinguish
default recipes, random blueprints, special-vendor blueprints, and faction
recipe vendors. Augments show the officially
...
ratings. Grade floors are S++ 24, S+ 18, S 14, A 10, B 6, C 3, and D 1;
anything below 1 is F. The
detail pane shows the adjusted effective score, base score, adjustment factor,
raw linear weight total, and coverage.

## Export Grades installation view

The Export Grades page derives `settings/text_en` from the Grim Dawn folder in
Settings. An existing localization directory is the primary source so Rainbow
colors and unrelated files are preserved; Grim Gleaner's complete bundled item
tags fill missing base or expansion files. It clones all files because partial
`tags_items` overrides make unmapped game tags display as `Tag not found`. Only
exact localization tags from compiled scoring catalogs are modified.
...
Export confirms the number of affix and unique-item entries before writing. The
first export for each configured installation stores its original `text_en`
state under Grim Gleaner's backups directory; re-exports do not overwrite that
snapshot. Restore Backups restores and consumes the snapshot. If `text_en` did
not exist originally, restore removes the folder generated by Grim Gleaner.
